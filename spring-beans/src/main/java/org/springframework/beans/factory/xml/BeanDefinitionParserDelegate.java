...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		// 对bean元素的详细解析  此处得到的beanDefinition是包含了所有bean信息的对象
		// 解析完成后要往beanDefinitionMap<beanName, beanDefinition>和beanDefinitionNames<beanName>放置值
		...
		if (beanDefinition != null) {
			...
			// 组装BeanDefinitionHolder返回，就是一个BeanDefinition的包装类
	...
	public void parseLookupOverrideSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			// 仅当在spring默认的bean的子元素下且为<lookup-method>时有效
	...
	public void parsePropertyElement(Element ele, BeanDefinition bd) {
		...
		try {
			...
			// 此处用来解析property值，返回的对象对应bean定义的property属性设置的解析结果，这个解析结果会封装到PropertyValue对象中，然后
