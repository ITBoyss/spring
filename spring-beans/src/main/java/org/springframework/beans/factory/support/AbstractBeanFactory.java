...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		// 提取相应的beanName，为什么不直接使用name，而是要经过一层转换呢？
		String beanName = transformedBeanName(name);
		Object beanInstance;

		// Eagerly check singleton cache for manually registered singletons.
		/**
		 * 根据beanName查看当前容器中是否已经存在创建好的bean（bean存储在三级缓存中）
		...
		if (sharedInstance != null && args == null) {
			...
			// 返回对象的实例，当实现了FactoryBean的接口的对象，需要获取具体的对象的时候就需要此方法来进行获取了
			...
			// 当对象是单例的时候会尝试解决循环依赖的问题，但是原型模式下如果存在循环依赖的情况，那么直接抛出异常
			...
			try {
				...
				// 此处做了BeanDefinition对象的类型转换，当我们从xml文件中加载beanDefinition对象的时候，
				// 封装的对象是GenericBeanDefinition，此处做类型转换，如果是子类Bean的话，会合并父类的相关属性
				...
				// 创建bean的实例对象
				if (mbd.isSingleton()) {
					// getSingleton传参，第二个参数lamda表达式，相当于一个匿名内部类
	...
	protected Object getObjectForBeanInstance(
		...
		// 如果指定的name是工厂相关并且beanInstance又不是FactoryBean类型则验证不通过
		if (BeanFactoryUtils.isFactoryDereference(name)) {
